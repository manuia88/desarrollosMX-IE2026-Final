...
| **11.S Living Atlas SEED** | ✅ **shipped** | **2071e9f** | **#32** |
...
- **2026-04-24 tarde — 11.S Living Atlas merged (PR #32).** Wiki colectiva 200 colonias CDMX seeded vía Claude Haiku 4.5 (cost cap $3 USD hard; proyección $1.73) + renderer markdown seguro (react-markdown + remark-gfm + rehype-sanitize, ADR-028) + SEO slugs desacoplados en nueva tabla `zone_slugs` + RLS relax `colonia_wiki_entries.published=true` (editorial queue diferido FASE 12 L-NEW8). **Desviación arquitectónica resuelta escalable**: founder pidió "ALTER TABLE zones ADD COLUMN slug" pero NO existe tabla master `zones` H1 (zone_id es UUID ref libre). Decisión: nueva tabla normalizada `zone_slugs (zone_id, scope_type, slug, country_code, source_label)` multi-country + multi-scope — versionable, indexable, zero dependency sobre tablas downstream. Filosofía `arquitectura_escalable_desacoplada` reafirmada. 2 migrations: `zone_slugs` + `audit_rls_allowlist_v25`. Registry 137 → 138 entries. Tests 2531 → 2574 (+43). Cross-functions: Atlas × Genoma (link /indices/DMX-LIV/similares) + Atlas × Climate Twin (link /indices/DMX-LIV/clima-gemelo). L-NEW10 admin-ext cleanup shipped mismo PR (cast transitional eliminado post db:types regen). **Lección canonizada**: ritual pre-prompt PM debe incluir "validar tabla master existe" (no solo columnas) — ahorró 30 min rework al detectar zone_slugs como decoupled design correcto antes del audit post-CC.
