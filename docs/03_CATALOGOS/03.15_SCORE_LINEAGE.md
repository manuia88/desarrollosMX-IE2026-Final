...
### LIVING_ATLAS — pipeline (BLOQUE 11.S)

```
LIVING_ATLAS (level 5, tier 3, agregado público editorial)
  │
  ▼ fetchColoniaSources (zona_snapshots MX con payload.name/label)
     query: country_code='MX' + order computed_at DESC + limit = target×3
     dedupe by zone_id + extract label (payload.name || payload.label)
     → ReadonlyArray<ColoniaSource{zone_id, label, alcaldia, country_code}>
     H1 requiere ingesta previa (FASE 07) — pipeline fail-safe si 0 rows
  │
  ▼ fetchExistingPublishedVersions (idempotencia)
     query: colonia_wiki_entries WHERE colonia_id IN (...) AND published=true
     → Map<colonia_id, max(version)> — skip colonias ya publicadas
  │
  ▼ fetchExistingSlugs (dedupe slug collision)
     query: zone_slugs → { slugSet: Set<slug>, zoneSlugs: Map<zone_id, slug> }
  │
  ▼ estimateBatchCostUsd (guard PRE-batch)
     costProj = colonias × (900 input_tokens_per × $0.80 + 2200 output_tokens_per × $4.00) / 1M
     if costProj > $3.00 → THROW abort (pre-cero LLM call)
  │
  ▼ for each colonia (sequential, batch ~200 rpm aprox):
       │
       ├── buildSystemPrompt() + buildUserPrompt(colonia)
       │     → Claude Haiku 4.5 (claude-haiku-4-5-20251001) max_tokens=3500
       │
       ├── extractJsonPayload(response.text) → parse tolerante {...}
       │
       ├── wikiSectionsOutputSchema.parse (Zod runtime validation)
       │     8 secciones: intro / historia / caracter / transporte /
       │     gastronomia / vida_cultural / seguridad_vida / mercado_inmobiliario
       │     Cada sección: string 80..1800 chars (min/max enforced)
       │
       ├── assembleContentMd(sections, label) → "# {label}\n\n## ... "
       │
       ├── report.inputTokens += usage.input_tokens
       │   report.outputTokens += usage.output_tokens
       │   runningCost = (inputTokens × $0.80 + outputTokens × $4.00) / 1M
       │
       ├── if runningCost > $3.00 → BREAK loop (cost guard running)
       │
       ├── upsertWikiEntry (colonia_wiki_entries):
       │     colonia_id + version = prevVersion + 1 (desde existingVersions map)
       │     content_md + sections jsonb + published=true + reviewed=false
       │     onConflict (colonia_id, version) idempotente
       │
       └── upsertSlug (zone_slugs):
             slug = existingSlugs.get(zone_id) ?? ensureUniqueSlug(slugify(label), slugSet)
             slugify: lowercase + strip diacritics + kebab-case + max 60 chars
             ensureUniqueSlug: suffix -N counter si collision
             onConflict (zone_id) idempotente
  │
  ▼ consumers (tRPC atlas router + pages):
    - trpc.atlas.getByColoniaSlug({slug}) → WikiEntry | null
        lookup: zone_slugs → colonia_wiki_entries (published=true, latest version)
        parseSections jsonb → ReadonlyArray<WikiSection>
    - trpc.atlas.listPublishedColonias({countryCode, limit}) → AtlasListedColonia[]
        join colonia_wiki_entries (published=true) × zone_slugs
    - UI /{locale}/atlas landing (Server Component) — lista colonias agrupadas
    - UI /{locale}/atlas/[coloniaSlug] detail (Server Component):
        WikiSectionList con TOC + smooth scroll + anchors
        WikiContentRenderer (react-markdown + remarkGfm + rehypeSanitize, ADR-028)
        JSON-LD Article structured data + alternates 5 locales canonical
        Sidebar cross-links:
          CF-11.S-1 → /indices/DMX-LIV/similares?scope_id=<coloniaId> (11.M genoma)
          CF-11.S-2 → /indices/DMX-LIV/clima-gemelo?scope_id=<coloniaId> (11.P climate twin)

granularidad one-shot seed (no cron) · costo proyectado 200 colonias ~$1.73 USD < cap $3
XSS safety: rehype-sanitize default schema bloquea javascript: / <script> / on* handlers / <iframe>
RLS relax colonia_wiki_public_read: using (published = true) — reviewed=true gate reactivado L-NEW8 FASE 12 N5
reemplazable FASE 12+: editor Tiptap/Lexical (L-NEW8) · sections normalizadas (L-NEW9 si >5k colonias)
```

### Resumen append 11.S

- **Scores agregados nuevos:** 1 (LIVING_ATLAS), level 5 tier 3 agregado. Total registry: **138 entries**.
- **Tablas persistencia:** `colonia_wiki_entries` (schema-native existente, RLS relajado a `published=true`) + `zone_slugs` (nueva migration 20260424200000, multi-country + multi-scope desacoplada) + allowlist v25 (cubre zone_slugs + policy change).
- **Escalabilidad:** `zone_slugs (zone_id UNIQUE, slug UNIQUE)` indexed + CHECK kebab-case regex — lookup O(log N) ambos sentidos; no dependency sobre tablas downstream. Idempotencia upsert `onConflict=zone_id` + `onConflict=colonia_id,version`. Seed cost proporcional N × $0.0086 USD (hard cap $3 = ≤348 colonias — holgura para expansion nacional).
- **Methodology versioning:** `methodology='haiku_v1'` implícito en content source. Reemplazable FASE 12 N5 (L-NEW8 editor Tiptap/Lexical + review queue humano — reactiva gate `reviewed=true`) y FASE 13 (L-NEW9 sections normalizadas si escala nacional >5k colonias).
- **Cross-function CF-11.S-1 / CF-11.S-2:** Atlas × Genoma (sidebar link similares 11.M) + Atlas × Climate Twin (sidebar link clima-gemelo 11.P) — on-demand links reutilizando engines existentes, zero migration, zero schema change.
- **Desviación arquitectónica resuelta:** founder pidió `ALTER zones ADD slug` pero no existe tabla master `zones` H1. Decisión: `zone_slugs` desacoplada normalizada. Filosofía `arquitectura_escalable_desacoplada` reafirmada. Ritual pre-prompt PM canonizado: "validar tabla master existe, no solo columnas".

**Autor append 11.S:** Manu Acosta + Claude Opus 4.7 | **Fecha:** 2026-04-24 tarde | **Shipped:** main SHA 2071e9f (PR #32).

---

## Append FASE 07.5.C-E — Lineage pulse + DNA + wiki + explainability cost tracking

Shipped 2026-04-24 (tag `fase-07.5-ingesta-canonical-complete`). Data lineage completo FASE 07.5.

### Pulse Score v1 (07.5.C)

- **Entregable:** `zone_pulse_scores` (daily 12m) + `pulse_forecasts` (H+30d).
- **Upstream:** `inegi_census_zone_stats.profession_distribution` · `enigh_zone_income.median_salary_mxn` · `zone_scores` (N0) · `zones` (geometry).
- **Compute script:** `scripts/compute/07_compute-zone-pulse.ts` (daily batch) + `scripts/compute/09_compute-zone-pulse-forecasts.ts` (H+30d rolling).
- **Methodology:** `pulse_v1` (weighted composite business_births + foot_traffic + events; forecast=moving average 90d + trend projection).
- **Explainability:** `zone_pulse_scores.components` (jsonb con contributions per signal).

### Colonia DNA Vector 64-dim + top_5 (07.5.C)

- **Entregable:** `colonia_dna_vectors.vector (64-dim)` + `components` jsonb (top_5 contributors with weights).
- **Upstream:** `dmx_indices` (14 códigos) · `zone_scores` (N0/N1) · `inegi_census_zone_stats` · `enigh_zone_income`.
- **Compute script:** `scripts/compute/08_compute-colonia-dna.ts`.
- **Methodology:** `dna_v1` (feature concatenation normalized + PCA dim reduction to 64-dim).
- **Index:** HNSW pgvector cosine (<50ms p95 twin lookup).
- **Explainability:** `components.top_5` = [{feature, contribution, value}] per vector — habilita "esta colonia se parece a X porque comparten Y, Z, W".

### Atlas Wiki Haiku 4.5 LLM (07.5.E U-E-1+U-E-2+U-E-4)

- **Entregable:** `colonia_wiki_entries.content_md` + `sections (jsonb, 8 secciones)` + `sections.meta (explainability)`.
- **Upstream consultado per entry (11 tablas):** `zones` · `zone_scores` · `dmx_indices` · `zone_pulse_scores` · `pulse_forecasts` · `colonia_dna_vectors` · `ghost_zones_ranking` · `climate_zone_signatures` · `zone_constellations_edges` · `inegi_census_zone_stats` · `enigh_zone_income`.
- **LLM:** Claude Haiku 4.5 (`claude-haiku-4-5-20251001`) via Anthropic SDK 0.90.0. Temperature 0.3. Max output tokens 3000.
- **Prompt caching (U-E-1):** `system = [WIKI_SYSTEM_PROMPT, WIKI_SCHEMA (cache_control:ephemeral), WIKI_EXAMPLES_3 (cache_control:ephemeral)]`. Status: **0% cache hit rate post-execution** (L-NEW46 + L-NEW49 tracker).
- **Cost tracking (U-E-2):** `sections.meta.token_usage` + `sections.meta.cost_usd` per entry. Run shipped 07.5.E: $1.74 total / $3 budget cap (cost real 3× proyección por caching miss — L-NEW49 fix pending).
- **Explainability (U-E-2):** `sections.meta.sources_consulted` + `facts_cited` + `generation_seed` per entry.
- **Methodology version:** `haiku_v1`. Reemplazable FASE 12 N5 (L-NEW8 editor Tiptap + review queue) / FASE 22 Multimedia (L-NEW45 multi-locale sentiment-aware).

### Climate Signatures + Twin Matching v1 (07.5.D)

- **Entregable:** `climate_monthly_aggregates` (228 × 192 meses) + `climate_annual_summaries` + `climate_zone_signatures` (vector 12-dim) + `climate_twin_matches` (top 5 per zone).
- **Methodology:** `heuristic_v1` (synthetic monthly aggregates determinísticos seed=zone_id+year+month · anomaly flags ±2σ).
- **Index:** HNSW cosine sobre `climate_zone_signatures.signature` vector(12).
- **RPC DB-side:** `find_climate_twins(zone_id, top_n)` → sub-lineal O(log N) escalable.

### Constellations + Ghost Transition v1 (07.5.D)

- **Entregable:** `zone_constellations_edges` (21,945 pairs × 4 edge types) + `zone_constellation_clusters` + `zone_topology_metrics` + `ghost_zones_ranking` + `transition_probability`.
- **Compute:** `11_compute-constellations-edges.ts` (Louvain approx + PageRank iterative 10 rounds damping 0.85) + `12_compute-ghost-zones.ts` (composite ghost_score 0-100 + logistic regression 12m transition probability).
- **Methodology:** `constellations_v1` (4 edge types: demographic_flow + economic_complement + cultural_affinity + spatial_adjacency) + `ghost_v1` (ghost_score = pulse_trend 30% + price_stagnant 25% + demographics_aging 20% + occupancy_low 25%).
- **Edge threshold:** `edge_weight >= 0.3` para evitar combinatorial explosion (228 × 228 = 51,984 pairs sin threshold).

### Canonical data sources (ingest_allowed_sources) — 56 total

FASE 07.5 consolidó **56 sources granulares** (4 INEGI + 6 compute IE/DMX + 3 compute pulse/dna/forecasts + 4 climate/constellations/ghost/topology + 1 atlas wiki + 2 audit/validation 07.5.F + resto heredado). Catalogación completa vía `SELECT source FROM ingest_allowed_sources ORDER BY source`.

### FK enforcement diferido (L-NEW13 FASE 08)

**Estado 07.5.F:** 0 FKs enforced entre 89 tablas con `zone_id`/`colonia_id` y `zones.id`. Intencional (evita cascade breakage mid-Opción D). Post-FASE 08 ship batch `ALTER TABLE ADD FOREIGN KEY` con cascades `ON DELETE` explícitos per dominio.

**Autor append 07.5.F:** Manu Acosta + Claude Opus 4.7 | **Fecha:** 2026-04-24 | **Shipped:** tag `fase-07.5-ingesta-canonical-complete`.
