...
import { easybrokerAdapter } from '@/features/marketing/lib/portals/easybroker';
...
export async function POST(request: Request): Promise<Response> {
  try {
    const expectedSecret = process.env.PORTAL_WEBHOOK_EASYBROKER_SECRET ?? '';
    const auth = request.headers.get('x-easybroker-signature') ?? '';
    if (expectedSecret && auth !== expectedSecret) {
      return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    }

    const payload = (await request.json().catch(() => null)) as unknown;
    const lead = easybrokerAdapter.parseLeadWebhook(payload);
    ...
    if (!zoneId) {
      sentry.captureException(
        new Error('easybroker-webhook: cannot resolve zone for incoming lead'),
        {
          tags: { feature: 'portal-webhook', portal: 'easybroker' },
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        metadata: { portal: 'easybroker', external_id: lead.projectId ?? null },
      })
      .select('id')
      .single();
    ...
    sentry.captureException(err, { tags: { feature: 'portal-webhook', portal: 'easybroker' } });
