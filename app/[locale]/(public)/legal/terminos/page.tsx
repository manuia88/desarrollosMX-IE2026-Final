...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const languages = Object.fromEntries(
    locales.map((l) => [l, `/${l}/legal/terminos`] as const),
...
export default async function TerminosPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    '@type': 'CreativeWork',
    name: t('terminos.page_title'),
    description: t('terminos.meta_description'),
    inLanguage: locale,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {t('terminos.back_home')}
        </Link>
        <span aria-hidden="true" style={{ margin: '0 var(--space-2, 0.5rem)' }}>
          /
        </span>
        <span>{t('terminos.breadcrumb')}</span>
      </nav>

      <div
        role="note"
        aria-label={t('terminos.draft_badge')}
        style={{
      ...
      >
        {t('terminos.draft_badge')}
      </div>

      <article aria-label={t('terminos.page_title')}>
        <MarkdownContentRenderer content={content} ariaLabel={t('terminos.page_title')} />
      </article>
    </main>
  );
}
