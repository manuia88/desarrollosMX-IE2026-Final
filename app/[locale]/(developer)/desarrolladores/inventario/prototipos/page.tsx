...
import { PrototiposManager } from '@/features/developer/components/inventario/PrototiposManager';
...
export async function generateMetadata({ params }: RouteProps) {
  ...
  return { title: `${t('subnav.prototipos')} · ${t('metaTitle')}` };
}

export default async function PrototiposRoute({ params }: RouteProps) {
  const { locale } = await params;
  ...
  if (!user) {
    redirect(`/${locale}/auth/login?next=/${locale}/desarrolladores/inventario/prototipos`);
  }
  ...
  return (
    <div className="space-y-4">
      <SubNav locale={locale} active="prototipos" t={t} />
      <PrototiposManager locale={locale} />
    </div>
  );
}
