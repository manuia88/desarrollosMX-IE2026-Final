...
import { EsquemasPagoManager } from '@/features/developer/components/inventario/EsquemasPagoManager';
...
export async function generateMetadata({ params }: RouteProps) {
  ...
  return { title: `${t('subnav.esquemasPago')} · ${t('metaTitle')}` };
}

export default async function EsquemasPagoRoute({ params }: RouteProps) {
  const { locale } = await params;
  ...
  if (!user) {
    redirect(`/${locale}/auth/login?next=/${locale}/desarrolladores/inventario/esquemas-pago`);
  }
  ...
  return (
    <div className="space-y-4">
      <SubNav locale={locale} active="esquemasPago" t={t} />
      <EsquemasPagoManager locale={locale} />
    </div>
  );
}
