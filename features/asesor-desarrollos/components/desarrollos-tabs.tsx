...
export interface DesarrollosTabsProps {
  tabCounts?: Partial<Record<TabKey, number>>;
}

export function DesarrollosTabs({ tabCounts }: DesarrollosTabsProps) {
  const t = useTranslations('AsesorDesarrollos.tabs');
  const { tab, setTab } = useDesarrollosTab();
      ...
      {TAB_KEYS.map((key) => {
        ...
        return (
          <button
            ...
            aria-controls="desarrollos-grid"
            onClick={() => setTab(key)}
            style={buttonStyle}
          >
            <span>{t(key)}</span>
