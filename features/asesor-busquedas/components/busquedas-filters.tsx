...
import { operacionEnum, tipoEnum } from '../lib/filter-schemas';

export function BusquedasFilters() {
  const t = useTranslations('AsesorBusquedas.filters');
  const { filters, setFilter, clear, hasActiveFilters } = useBusquedasFilters();
  ...
  return (
    <section style={containerStyle} aria-label={t('ariaSearch')}>
      ...
      >
        ...
        <option value="departamento">{t('tipoOption.departamento')}</option>
        <option value="casa">{t('tipoOption.casa')}</option>
        <option value="terreno">{t('tipoOption.terreno')}</option>
        <option value="oficina">{t('tipoOption.oficina')}</option>
        <option value="local">{t('tipoOption.local')}</option>
      </select>
      ...
      <select
        ...
        onChange={(e) => {
          const v = e.target.value;
          setFilter('operacion', v ? operacionEnum.parse(v) : undefined);
        }}
        style={dropdownStyle}
      >
