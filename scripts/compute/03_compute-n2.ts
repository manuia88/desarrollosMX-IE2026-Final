#!/usr/bin/env node
/**
 * Compute batch N2 — itera zones CDMX × scores {level=2, category='zona', MX}
 * y persiste via runScore() → persistZoneScore built-in. Log en ingest_runs
 * via withIngestRun.
 *
 * Uso:
 *   SUPABASE_SERVICE_ROLE_KEY=... NEXT_PUBLIC_SUPABASE_URL=... \
 *     node --experimental-strip-types scripts/compute/03_compute-n2.ts
 *
 * Flags:
 *   --dry-run             Solo lista scores y cuenta zones. NO calcula ni persiste.
 *   --limit=N             Procesa sólo N zones (default: ALL).
 *   --country=MX          Country code (default: MX — único soportado H1).
 */

import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { createClient, type SupabaseClient } from '@supabase/supabase-js';
import { registerN2Calculators } from '../../shared/lib/intelligence-engine/calculators/n2/index.ts';
...
const SOURCE = 'compute_ie_n2';
const TRIGGERED_BY = 'cli:compute-n2';
  ...
  for (const a of argv.slice(2)) {
    if (a === '--dry-run') {
      ...
      if (!Number.isFinite(n) || n <= 0) {
        throw new Error(`[compute-n2] --limit inválido: "${a}"`);
      }
      limit = n;
    } else if (a.startsWith('--country=')) {
      const raw = a.slice('--country='.length).trim();
      if (raw.length === 0) {
        throw new Error('[compute-n2] --country vacío');
      }
      country = raw;
    }
  }
...
export function requireEnv(name: string): string {
  ...
  if (v == null || v === '') {
    throw new Error(`[compute-n2] Falta env var requerida: ${name}. Exportala antes de correr.`);
  }
  return v;
}
...
async function fetchZones(
  ...
  if (error) {
    throw new Error(`[compute-n2] fetch zones: ${error.message}`);
  }
    ...
    for (const entry of targetScores) {
      try {
        ...
        if (result.kind === 'ok') {
          ...
          counts.dlq += 1;
          console.error(
            `[compute-n2] error score_id=${entry.score_id} scope_id=${zone.scope_id}: ${result.error}`,
          );
        }
      } catch (err) {
        counts.dlq += 1;
        ...
        console.error(
          `[compute-n2] thrown score_id=${entry.score_id} scope_id=${zone.scope_id}: ${msg}`,
        );
      }
    }
  }
...
async function main(): Promise<number> {
  ...
  if (targetScores.length === 0) {
    console.log(
      `${tag} no zona-category scores at level ${LEVEL}; skipping. writing empty success run`,
    );
  }
...
if (invokedAsScript) {
  main()
    ...
    .catch((err) => {
      console.error('[compute-n2] FATAL:', err);
      process.exit(1);
    });
}
