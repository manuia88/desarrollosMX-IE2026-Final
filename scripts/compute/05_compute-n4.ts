#!/usr/bin/env node
/**
 * Compute batch N4 — itera zones CDMX × scores {level=4, category='zona', MX}
 * y persiste via runScore() → persistZoneScore built-in. Log en ingest_runs
 * via withIngestRun.
 *
 * Uso:
 *   SUPABASE_SERVICE_ROLE_KEY=... NEXT_PUBLIC_SUPABASE_URL=... \
 *     node --experimental-strip-types scripts/compute/05_compute-n4.ts
 *
 * Flags:
...
import { registerN4Calculators } from '../../shared/lib/intelligence-engine/calculators/n4/index.ts';
...
const SOURCE = 'compute_ie_n4';
const TRIGGERED_BY = 'cli:compute-n4';
  ...
  for (const a of argv.slice(2)) {
    if (a === '--dry-run') {
      ...
      if (!Number.isFinite(n) || n <= 0) {
        throw new Error(`[compute-n4] --limit inválido: "${a}"`);
      }
      limit = n;
    } else if (a.startsWith('--country=')) {
      const raw = a.slice('--country='.length).trim();
      if (raw.length === 0) {
        throw new Error('[compute-n4] --country vacío');
      }
      country = raw;
    }
  }
...
export function requireEnv(name: string): string {
  ...
  if (v == null || v === '') {
    throw new Error(`[compute-n4] Falta env var requerida: ${name}. Exportala antes de correr.`);
  }
  return v;
}
...
async function fetchZones(
  ...
  if (error) {
    throw new Error(`[compute-n4] fetch zones: ${error.message}`);
  }
    ...
    for (const entry of targetScores) {
      try {
        ...
        if (result.kind === 'ok') {
          ...
          counts.dlq += 1;
          console.error(
            `[compute-n4] error score_id=${entry.score_id} scope_id=${zone.scope_id}: ${result.error}`,
          );
        }
      } catch (err) {
        counts.dlq += 1;
        ...
        console.error(
          `[compute-n4] thrown score_id=${entry.score_id} scope_id=${zone.scope_id}: ${msg}`,
        );
      }
    }
  }
...
if (invokedAsScript) {
  main()
    ...
    .catch((err) => {
      console.error('[compute-n4] FATAL:', err);
      process.exit(1);
    });
}
