#!/usr/bin/env node
/**
 * Compute batch IE — Nivel 0 (category='zona', country='MX') sobre todas las
 ...
 * completa vía withIngestRun (source='compute_ie_n0').
 *
 * Uso:
 *   SUPABASE_SERVICE_ROLE_KEY=... NEXT_PUBLIC_SUPABASE_URL=... \
 *     node --experimental-strip-types scripts/compute/01_compute-n0.ts
 *
 * Flags:
...
import { registerN0Calculators } from '../../shared/lib/intelligence-engine/calculators/n0/index.ts';
import { registerN01ToN11Calculators } from '../../shared/lib/intelligence-engine/calculators/n01-n11/index.ts';
...
const SOURCE = 'compute_ie_n0';
const SCRIPT_NAME = '01_compute-n0.ts';
  ...
  for (const a of argv.slice(2)) {
    if (a === '--dry-run') {
      ...
      if (!Number.isFinite(n) || n <= 0) {
        throw new Error(`[compute-n0] --limit inválido: "${a}"`);
      }
      limit = n;
    } else if (a.startsWith('--country=')) {
      const raw = a.slice('--country='.length).trim();
      if (raw.length !== 2) {
        throw new Error(`[compute-n0] --country inválido (ISO-3166-1 alpha-2): "${a}"`);
      }
...
function requireEnv(name: string): string {
  ...
  if (v == null || v === '') {
    throw new Error(`[compute-n0] Falta env var requerida: ${name}. Exportala antes de correr.`);
  }
  return v;
}
    ...
    for (const reg of targetScores) {
      try {
        ...
        if (res.kind === 'ok') {
          ...
          counts.dlq += 1;
          console.error(
            `[compute-n0] err ${reg.score_id}/${zone.scope_id}: ${res.kind === 'error' ? res.error : 'unknown'}`,
          );
        }
      } catch (e) {
        counts.dlq += 1;
        console.error(
          `[compute-n0] throw ${reg.score_id}/${zone.scope_id}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
  }
...
async function main(): Promise<number> {
  ...
  if (args.dryRun) {
    console.log(
      `[compute-n0] DRY RUN — would process ${targetScores.length} scores × zones (limit=${args.limit}, country=${args.country})`,
    );
  ...
  const { data: zones, error } = await supabase
    ...
    .limit(args.limit);
  if (error || !zones) {
    throw new Error(`[compute-n0] zones fetch failed: ${error?.message ?? 'unknown'}`);
  }
  ...
  console.log(
    `[compute-n0] done: status=${result.status} counts=${JSON.stringify(result.counts)} duration_ms=${result.durationMs}`,
  );
