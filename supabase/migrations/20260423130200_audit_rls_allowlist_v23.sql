-- audit_rls_allowlist v23 — FASE 11 FIX 11.O+P.
--
-- Clona v22. Cambios cubiertos:
--   11.P FIX  DROP+CREATE climate_annual_summaries (pgvector(12) + HNSW).
--             Policies nuevas con comment intentional_public (reemplazan las previas drop).
--   11.P FIX  Nueva tabla climate_zone_signatures (pgvector(12) + HNSW).
--             Policies con comment intentional_public.
--   11.P FIX  ALTER climate_monthly_aggregates.source check/default (heuristic_v1).
--             No policies nuevas.
--   11.P FIX  Nueva function find_climate_twins(uuid, int, numeric)
--             SECDEF stable + set search_path (añadida al exclude list).
...
comment on function public.audit_rls_violations() is
  'v23 — FASE 11 FIX 11.O+P: climate_annual_summaries refactor pgvector(12)+HNSW, '
  'nueva climate_zone_signatures (pgvector+HNSW), ALTER source heuristic_v1 default, '
  'find_climate_twins RPC (stable, SECDEF read-only, search_path locked).';
