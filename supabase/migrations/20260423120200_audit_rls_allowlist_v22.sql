-- audit_rls_allowlist v22 — FASE 11 BLOQUES 11.O + 11.P.
--
-- Clona v21. Cambios cubiertos:
--   11.O  ALTER lifepath_user_profiles (+answers_version, +methodology, rename top_3_matches→matches).
--         No introduce policies nuevas — las existentes owner-based siguen válidas.
--   11.P  RENAME climate_twin_projections → climate_future_projections
--         (policies renombradas climate_future_public_read / climate_future_service_write —
--         el comment intentional_public persiste porque ALTER POLICY RENAME no altera comments).
--   11.P  3 nuevas tablas públicas (climate_monthly_aggregates, climate_annual_summaries,
--         climate_twin_matches) — todas cubiertas por policy comment intentional_public.
--
-- No nuevos SECDEF (calculators corren desde Node/Next vía service_role).
...
comment on function public.audit_rls_violations() is
  'v22 — FASE 11 BLOQUES 11.O + 11.P: ALTER lifepath_user_profiles (no policies nuevas). '
  'RENAME climate_twin_projections → climate_future_projections (policies renombradas, comment intentional_public persiste). '
  '3 tablas históricas nuevas (climate_monthly_aggregates, climate_annual_summaries, climate_twin_matches) con comment intentional_public.';
