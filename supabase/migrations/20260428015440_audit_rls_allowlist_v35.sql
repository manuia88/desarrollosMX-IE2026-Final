-- audit_rls_allowlist v35 — F14.F.7 Sprint 6 BIBLIA Seedance + Virtual Staging + Drone.
--
-- v35 amplía v34 con justificación de policy "qual=true" en studio_audio_ambient_library:
--   - studio_audio_ambient_library_select_authenticated (qual=true)
--
-- Razón excluida del POLICY_QUAL_TRUE_UNJUSTIFIED check:
--   Lectura libre para asesores autenticados — biblioteca canon 12 ambientes audio
--   (ocean_view, kitchen_modern, garden_birds, ...). Datos no sensibles, mismos
--   para todos los users (catálogo seed). Acceso reducido a `to authenticated`
--   role (no público anonymous), policy aplica solo a SELECT.
--   Se marca via COMMENT ON POLICY con keyword 'intentional_public_authed'.
--
-- 1:1 verification SECDEF↔allowlist (memoria audit_rls_strict_post_merge_gap):
--   v35 NO añade SECDEF nueva. Solo extiende justification policy mediante COMMENT.
--   Body de audit_rls_violations() permanece idéntico a v34 (re-create idempotente
--   para version bump documental).

comment on policy studio_audio_ambient_library_select_authenticated
  on public.studio_audio_ambient_library is
  'intentional_public_authed: Catálogo seed canon (12 ambientes) lectura libre para todos los asesores autenticados. Datos no sensibles compartidos.';
...
comment on function public.audit_rls_violations() is
  'v35 - F14.F.7 Sprint 6 BIBLIA Seedance + Virtual Staging + Drone. Amplia v34 con justification policy studio_audio_ambient_library_select_authenticated via COMMENT intentional_public_authed (catálogo seed 12 ambientes lectura libre asesores).';
