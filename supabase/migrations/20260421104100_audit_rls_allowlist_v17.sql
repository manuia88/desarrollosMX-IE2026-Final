-- audit_rls_allowlist v17 — FASE 11 XL BLOQUE 11.C.
--
-- Clona v16 + agrega 3 cascade functions para DMX-* índices al allowlist
-- SECDEF_NO_AUTH_CHECK:
--   - fn_enqueue_indices_for_zone
--   - fn_trg_zone_scores_cascade_indices
--   - fn_trg_project_scores_cascade_indices
--
-- Las 3 son security definer plpgsql utilitarias que enqueue jobs a
-- score_recalculation_queue; no exponen datos al caller y no requieren
-- auth check (solo se invocan por triggers sobre zone_scores/project_scores).
...
begin
  ...
  where p.prosecdef and n.nspname = 'public'
    ...
    and p.proname not in (
      ...
      -- FASE 11 XL BLOQUE 11.C: cascade functions índices DMX.
      'fn_enqueue_indices_for_zone',
      'fn_trg_zone_scores_cascade_indices',
      'fn_trg_project_scores_cascade_indices'
    )
...
comment on function public.audit_rls_violations() is
  'v17 — FASE 11 XL BLOQUE 11.C: agrega 3 cascade functions para DMX-* índices.';
