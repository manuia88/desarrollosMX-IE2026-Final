-- audit_rls_allowlist v32 — F1.C.C Tier 2 Demographics (AGEB spatial overlay)
--
-- v32 amplía v31 con 1 SECDEF nueva F1.C.C:
--   - recompute_zone_demographics_from_ageb(uuid)
--
-- Razón excluida del SECDEF auth check:
--   Service-role-only batch RPC. PostGIS spatial overlay (ST_Intersects +
--   ST_Area weighted aggregation) sobre inegi_ageb_staging × zones.boundary
--   → UPSERT inegi_census_zone_stats con data_origin='inegi_ageb_overlay'.
--   Callable solo via tools/scripts/run-demographics-tier2-backfill.ts con
--   service_role key. Body validates staging populated + UPSERT-only (no
--   DELETE/DROP/cross-table cascades). search_path obligatorio
--   ('public, pg_catalog').
--
-- 1:1 verification SECDEF↔allowlist (memoria audit_rls_strict_post_merge_gap):
--   Migration F1.C.C añade exactamente 1 SECDEF nueva → v32 añade exactamente 1 entry.
...
begin
  ...
  where p.prosecdef and n.nspname = 'public'
    ...
    and p.proname not in (
      ...
      -- v31 additions F1.D Geometry Real (GeoJSON CDMX MGN):
      'update_zone_boundary_from_geojson',
      -- v32 additions F1.C.C Tier 2 Demographics (AGEB spatial overlay):
      -- recompute_zone_demographics_from_ageb: service-role-only batch RPC.
      -- PostGIS spatial overlay (ST_Intersects + ST_Area weighted aggregation)
      -- sobre inegi_ageb_staging × zones.boundary → UPSERT
      -- inegi_census_zone_stats con data_origin='inegi_ageb_overlay'.
      -- Callable solo via backfill script con service_role key. UPSERT-only,
      -- no DELETE/DROP/cross-table cascades. search_path='public, pg_catalog'.
...
comment on function public.audit_rls_violations() is
  'v32 — F1.C.C Tier 2 Demographics (AGEB spatial overlay). Amplía v31 con 1 SECDEF nueva: '
  'recompute_zone_demographics_from_ageb. Razón excluida: service-role-only batch RPC, '
  'PostGIS spatial overlay sobre inegi_ageb_staging × zones.boundary, UPSERT-only sin '
  'cross-table cascades. search_path=''public, pg_catalog'' obligatorio.';
