-- audit_rls_allowlist v33 — F1.C.C Tier 2 Demographics (AGEB batch loader)
--
-- v33 amplía v32 con 1 SECDEF nueva F1.C.C:
--   - load_inegi_ageb_staging_batch(jsonb)
--
-- Razón excluida del SECDEF auth check:
--   Service-role-only batch loader. INSERT/UPDATE inegi_ageb_staging desde
--   jsonb array (chunks 200 rows). Casts GeoJSON text → PostGIS geometry
--   via ST_GeomFromGeoJSON + ST_Multi + ST_MakeValid. Callable solo via
--   tools/scripts/run-demographics-tier2-backfill.ts con service_role.
--   UPSERT con on conflict (cve_ent, cve_mun, cve_loc, cve_ageb) — sin
--   DELETE/DROP/cross-table cascades. search_path='public, pg_catalog'.
--
-- 1:1 verification SECDEF↔allowlist (memoria audit_rls_strict_post_merge_gap):
--   Migration añade exactamente 1 SECDEF nueva → v33 añade exactamente 1 entry.
...
comment on function public.audit_rls_violations() is
  'v33 - F1.C.C Tier 2 Demographics (AGEB spatial overlay + batch loader). Amplia v32 con SECDEF nueva: load_inegi_ageb_staging_batch (jsonb-driven UPSERT staging). Service-role-only batch ingestion via tools/scripts/run-demographics-tier2-backfill.ts. Casts GeoJSON text to PostGIS via ST_GeomFromGeoJSON + ST_Multi + ST_MakeValid.';
