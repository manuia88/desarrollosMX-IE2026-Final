-- audit_rls_allowlist v12 — FASE 10 SESIÓN 1/3 correctivo.
-- v11 usó COMMENT ON FUNCTION para marker intentional_secdef, pero
-- audit_rls_violations() usa lista hardcodeada para SECDEF_NO_AUTH_CHECK.
-- Agrega fn_emit_score_change_webhook a la lista.
...
begin
  ...
  where p.prosecdef and n.nspname = 'public'
    ...
    and p.proname not in (
      ...
      -- FASE 09 D11: cascade N1↔N0 triggers SECDEF.
      'fn_cascade_score_updated',
      'fn_trg_zone_scores_cascade',
      'fn_trg_project_scores_cascade',
      'fn_trg_user_scores_cascade',
      -- FASE 10 SESIÓN 1/3 D21: webhook emitter AFTER UPDATE zone_scores.
      -- Trigger body, no entry point externo. Lee webhooks + encola deliveries.
...
comment on function public.audit_rls_violations() is
  'v12 — FASE 10 SESIÓN 1/3: allowlist fn_emit_score_change_webhook (D21 webhook emitter trigger body).';
