-- audit_rls_allowlist v18 — FASE 11 BLOQUE 11.J Newsletter.
--
-- Clona v17 + agrega las 4 tablas nuevas del schema Newsletter
-- (20260421110000_newsletter_schema):
--   - newsletter_subscribers (RLS on; 4 policies + service bypass)
--   - newsletter_deliveries  (RLS on; 1 owner read + service bypass)
--   - newsletter_ab_tests    (RLS on; public read + service bypass)
--   - dmx_wrapped_snapshots  (RLS on; public read + service bypass)
--
-- Policies con qual=true usadas:
--   - newsletter_ab_tests_public_read        (qual=true, comment intentional_public ✓)
--   - dmx_wrapped_public_read                (qual=true, comment intentional_public ✓)
--   - newsletter_subscribers_public_insert   (INSERT cmd='a' — excluida del check)
--   - service_all policies                   (role=service_role — excluidas por v16 filter)
--
-- No hay nuevas SECDEF functions — writers corren como service_role desde cron
-- + adapters tRPC; no requiere ampliar allowlist SECDEF_NO_AUTH_CHECK.
...
comment on function public.audit_rls_violations() is
  'v18 — FASE 11.J Newsletter: allowlist 4 tablas nuevas (newsletter_subscribers, '
  'newsletter_deliveries, newsletter_ab_tests, dmx_wrapped_snapshots). Policies '
  'intencionalmente públicas usan comment RATIONALE intentional_public; service_role '
  'policies excluidas por filtro v16. Lista SECDEF sin cambios vs v17.';
