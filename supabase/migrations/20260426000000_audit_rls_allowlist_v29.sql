-- audit_rls_allowlist v29 — FASE 07.7.A.4 CRM Foundation (completar v28)
--
-- v29 amplía v28 con 3 SECDEF triggers olvidados en A.4:
--   - crm_handle_deal_stage_change (trigger, cascade closed_won → operacion)
--   - crm_handle_operacion_insert (trigger, cascade commission calc)
--   - trg_behavioral_signals_update_heartbeat (trigger, FK enforcement + last_signal_at update)
--
-- Razón excluidas del SECDEF auth check:
--   SECDEF triggers RETURNS trigger auto-disparadas POST-RLS, no callable directly por user.
--   search_path='' en todas (defensa search path attack).
--   Cross-table cascades requieren SECDEF sin auth.uid() guard.
--
-- v28 documentaba estas 3 en comentario header pero olvidó agregarlas al array de exclusión
-- (líneas 17-20 v28). v29 corrige el gap. Detección: CI audit-rls strict en PR #60
-- (db-types-post-A.4) tras db push remoto manual de A.4.
...
begin
  ...
  where p.prosecdef and n.nspname = 'public'
    ...
    and p.proname not in (
      ...
      -- SECDEF triggers RETURNS trigger auto-disparadas POST-RLS, no callable directly user.
      -- search_path='' en todas. Cross-table cascades requieren SECDEF sin auth.uid() guard.
...
comment on function public.audit_rls_violations() is
  'v29 — FASE 07.7.A.4 CRM Foundation completar. Amplía v28 con 3 SECDEF triggers olvidados: '
  'crm_handle_deal_stage_change, crm_handle_operacion_insert, trg_behavioral_signals_update_heartbeat. '
  'Razón excluidas: SECDEF triggers RETURNS trigger auto-disparadas POST-RLS, no callable directly user. '
  'search_path='''' en todas. Cross-table cascades requieren SECDEF sin auth.uid() guard.';
