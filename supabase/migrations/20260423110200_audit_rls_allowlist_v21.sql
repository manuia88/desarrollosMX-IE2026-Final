-- audit_rls_allowlist v21 — FASE 11 BLOQUE 11.N (Futures banda + Pulse forecasts).
--
-- Clona v20. Cubre la nueva tabla pulse_forecasts (policy public_read qual=true
-- marcada intentional_public por comment). ALTER futures_curve_projections no
-- introduce policies nuevas — columnas banda existen bajo la policy existente
-- desde v15.
--
...
comment on function public.audit_rls_violations() is
  'v21 — FASE 11 BLOQUE 11.N: pulse_forecasts cubierto por policy comment intentional_public; ALTER futures bandas no añade policies (cubiertas desde v15).';
