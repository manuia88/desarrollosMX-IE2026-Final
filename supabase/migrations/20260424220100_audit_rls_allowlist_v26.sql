-- audit_rls_allowlist v26 — BATCH 5 pre-Opción D canonical catalog naming.
--
-- Clona v25. Cambio cubierto:
--   Tabla public.feature_registry renamed → public.ui_feature_flags
--   (migration 20260424220000). Rename coordinado:
...
--       adaptan auto (constraint names no incluyen feature_registry).
--
-- Función audit_rls_violations() NO hardcodea table/policy names — evalúa
-- pg_catalog dinámicamente — por tanto el rename es transparente al checker.
-- Bump a v26 es tracking version + actualización del comment.
--
...
comment on function public.audit_rls_violations() is
  'v26 — BATCH 5 pre-Opción D canonical catalog naming (ADR-029): '
  'public.feature_registry renamed → public.ui_feature_flags. '
  '4 policies + 1 trigger renamed (feature_registry_* → ui_feature_flags_*, '
  'trg_audit_feature_registry → trg_audit_ui_feature_flags). '
  'Función evalúa pg_catalog dinámicamente — rename transparente al checker.';
