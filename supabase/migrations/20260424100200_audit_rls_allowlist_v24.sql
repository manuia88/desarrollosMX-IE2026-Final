-- audit_rls_allowlist v24 — FASE 11 XL BLOQUE 11.R Zone Constellations.
--
-- Clona v23. Cambios cubiertos:
--   11.R.-1.1 Nuevos index zone_constellations_edges_types_gin + _weight_idx
--             (GIN + BTree — no cambian RLS).
--   11.R.-1.2 Nueva tabla zone_constellation_clusters (RLS enabled +
--             policies public_read con comment intentional_public +
--             service_write).
--   Sin nuevas functions SECDEF — el engine Louvain + path finder corren
--   en Node (shared/lib/intelligence-engine/constellations).
...
comment on function public.audit_rls_violations() is
  'v24 — FASE 11 XL BLOQUE 11.R Zone Constellations: nueva '
  'zone_constellation_clusters table (RLS + policies intentional_public), '
  'nuevos indexes sobre zone_constellations_edges (GIN + BTree weight). '
  'Sin SECDEF nuevas (engine Louvain + path finder corren en Node).';
