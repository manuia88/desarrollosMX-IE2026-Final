-- audit_rls_allowlist v25 — FASE 11 XL BLOQUE 11.S Living Atlas.
--
-- Clona v24. Cambios cubiertos:
--   11.S.-1 Nueva tabla public.zone_slugs (RLS enabled + policies
--           public_read con comment intentional_public + service_write).
--           Sin nuevas functions SECDEF — slug resolution corre en Node
--           (features/atlas/lib/slug-resolver.ts) usando service_role
--           client para escribir y anon client para leer.
--   11.S.-1 Policy colonia_wiki_public_read alterada (drop reviewed=true
--           requirement, keeps published=true). Comment intentional_public
--           preserved con rationale actualizado.
...
comment on function public.audit_rls_violations() is
  'v25 — FASE 11 XL BLOQUE 11.S Living Atlas: nueva zone_slugs table '
  '(RLS + policies intentional_public). Policy colonia_wiki_public_read '
  'alterada (drop reviewed=true requirement, keeps published=true). '
  'Sin SECDEF nuevas — slug resolver corre en Node.';
