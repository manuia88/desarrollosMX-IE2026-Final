-- audit_rls_allowlist v31 — F1.D Geometry Real (GeoJSON CDMX MGN)
--
-- v31 amplía v30 con 1 SECDEF nueva F1.D:
--   - update_zone_boundary_from_geojson(uuid, text, numeric, numeric, numeric)
--
-- Razón excluida del SECDEF auth check:
--   Service-role-only batch migration RPC. Atomic UPDATE zones.boundary +
--   area_km2 + lat/lng centroid from MGN GeoJSON polygon. Callable solo via
--   backfill script con service_role key. Body validates input + zone_id
--   existence + UPDATE-only (no DELETE/DROP/cross-table cascades).
--   search_path='' obligatorio.
--
-- 1:1 verification SECDEF↔allowlist (memoria audit_rls_strict_post_merge_gap):
--   Migration F1.D añade exactamente 1 SECDEF nueva → v31 añade exactamente 1 entry.
...
begin
  ...
  where p.prosecdef and n.nspname = 'public'
    ...
    and p.proname not in (
      ...
      -- recompute_climate_aggregates_from_observations: service-role-only
      -- orchestration RPC. Aggregates climate_source_observations en
      -- climate_monthly_aggregates. search_path=''. Callable via service role.
      'recompute_climate_aggregates_from_observations',
      -- v31 additions F1.D Geometry Real (GeoJSON CDMX MGN):
      -- update_zone_boundary_from_geojson: service-role-only batch migration RPC.
      -- Atomic UPDATE zones.boundary + area_km2 + lat/lng centroid from MGN GeoJSON.
      -- Callable solo via backfill script con service_role key. UPDATE-only,
      -- no DELETE/DROP/cross-table cascades. search_path=''.
...
comment on function public.audit_rls_violations() is
  'v31 — F1.D Geometry Real (GeoJSON CDMX MGN). Amplía v30 con 1 SECDEF nueva: '
  'update_zone_boundary_from_geojson. Razón excluida: service-role-only batch '
  'migration RPC, atomic UPDATE zones.boundary from MGN GeoJSON. UPDATE-only, '
  'search_path='''' obligatorio. Callable solo via backfill script con service_role.';
