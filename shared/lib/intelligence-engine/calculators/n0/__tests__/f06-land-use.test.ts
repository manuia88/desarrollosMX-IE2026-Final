...
import f06, { getLabelKey, methodology, reasoning_template, version } from '../f06-land-use';

describe('F06 Land Use stub', () => {
  it('declara version semver', () => {
    expect(version).toMatch(/^\d+\.\d+\.\d+/);
  });

  it('declara methodology + reasoning_template', () => {
    expect(methodology.sources).toContain('seduvi');
    expect(reasoning_template).toContain('{zona_name}');
  });

  it('getLabelKey siempre devuelve ie.score.f06.pending_h2', () => {
    expect(getLabelKey(0, 'insufficient_data')).toBe('ie.score.f06.pending_h2');
  });
  ...
  it('16 zonas CDMX — insufficient_data + score 0 + provenance válido', async () => {
    ...
    for (const zone of CDMX_ZONES) {
      ...
      expect(out.score_label, zone.zona_name).toBe('ie.score.f06.pending_h2');
