...
// throwea bbva_pdf_parser_not_installed. Los tests inyectan pdfParserImpl
...
async function defaultPdfParser(_buffer: Buffer): Promise<{ text: string; numpages?: number }> {
  // Intenta require dinámico — si el paquete no está presente (caso H1),
  // throwea código canónico que el admin-upload UI reconoce.
  try {
    ...
    if (!mod) throw new Error('bbva_pdf_parser_not_installed');
    ...
    if (e instanceof Error && e.message === 'bbva_pdf_parser_not_installed') throw e;
